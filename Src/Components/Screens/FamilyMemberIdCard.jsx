  ...
  if (!member) {
    return (
      <BackgroundWrapper>
        <SafeAreaView style={styles.loaderSafe}>
          <View style={styles.loaderContainer}>
            <Text>No family member data found</Text>
          </View>
        </SafeAreaView>
      </BackgroundWrapper>
    );
    ...
    try {
      ...
      qrRef.current.toDataURL(async (data) => {
        try {
                ...
                <div style="border:2px solid #1f8a43; border-radius:20px; overflow:hidden; background:#ffffff;">
                  <div style="background:#1f8a43; color:#ffffff; padding:20px 24px; display:flex; justify-content:space-between; align-items:flex-start;">
                    <div>
                      ...
                      <div style="font-size:18px; margin-top:6px;">Family Member ID Card</div>
                    </div>
                  ...
                  <div style="padding:24px; display:flex; justify-content:space-between; align-items:flex-start;">
                    <div style="width:58%;">
                      ...
                      <div style="font-size:18px; margin:6px 0 14px 0;">${member.fullName || ""}</div>

                      <div style="font-size:16px; font-weight:bold; color:#11c95c;">Relationship</div>
                      <div style="font-size:18px; margin:6px 0 14px 0;">${member.relationship || ""}</div>

                      <div style="font-size:16px; font-weight:bold; color:#11c95c;">Contact</div>
                      <div style="font-size:18px; margin:6px 0 14px 0;">${member.phone || ""}</div>

                      <div style="font-size:16px; font-weight:bold; color:#11c95c;">Gender</div>
                      <div style="font-size:18px; margin:6px 0 14px 0;">${member.gender || ""}</div>

                      <div style="font-size:16px; font-weight:bold; color:#11c95c;">Address</div>
                      <div style="font-size:18px; margin:6px 0 14px 0;">${member.address || ""}</div>

                      <div style="font-size:16px; font-weight:bold; color:#11c95c;">Date Of Birth</div>
                      <div style="font-size:18px; margin:6px 0 0 0;">${member.birthDate || ""}</div>
                    </div>
          ...
          await generatePDF({
            html,
            fileName: `family_member_id_card_${patientId}`,
            directory: "Documents",
          });
  ...
  return (
    <BackgroundWrapper>
      <SafeAreaView style={styles.safe}>
        ...
        >
          <Text style={styles.pageTitle}>Family Member ID Card</Text>

          <View style={styles.cardBox}>
            <View style={styles.cardHeader}>
              <View>
                <Text style={styles.brandText}>DentPulse</Text>
                <Text style={styles.subBrandText}>Family Member ID Card</Text>
              </View>
            ...
            <View style={styles.cardBody}>
              <View style={styles.leftSection}>
                ...
                <Text style={styles.fieldValue}>{member.fullName}</Text>

                <Text style={styles.fieldLabel}>Relationship</Text>
                <Text style={styles.fieldValue}>{member.relationship}</Text>
                ...
                <Text style={styles.fieldValue}>{member.birthDate}</Text>
              </View>
...
export default FamilyMemberIdCard;